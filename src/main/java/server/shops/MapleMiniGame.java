...
public class MapleMiniGame extends AbstractPlayerStore {
    ...
    public MapleMiniGame(MapleCharacter owner, int itemId, String description, String pass, int GameType, Point pos) {
        super(owner, itemId, description, pass, slots - 1, pos); //?
    ...
    public void setGameType() {
        if (GameType == 2 || GameType == 4) { //omok = 1
            matchcards.clear();
    ...
    public byte getShopType() {
        return GameType == 1 || GameType == 3 ? IMaplePlayerShop.OMOK : IMaplePlayerShop.MATCH_CARD;
    }
    ...
    public void setPoints(int i, int type) { //lose = 0, tie = 1, win = 2
        ...
        if (z != null) {
            ...
            z.getQuestNAdd(MapleQuest.getInstance(GameType == 1 || GameType == 3 ? GameConstants.OMOK_SCORE : GameConstants.MATCH_SCORE)).setCustomData(newDat.substring(0, newDat.length() - 1));
        }
    }

    public String getData(MapleCharacter chr) {
        MapleQuest quest = MapleQuest.getInstance(GameType == 1 || GameType == 3 ? GameConstants.OMOK_SCORE : GameConstants.MATCH_SCORE);
