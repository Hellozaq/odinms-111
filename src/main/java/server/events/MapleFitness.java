...
public class MapleFitness extends MapleEvent {
    ...
    public void onMapLoad(MapleCharacter chr) {
        super.onMapLoad(chr);
        if (isTimerStarted()) {
            chr.getClient().sendPacket(EtcPacket.getClock((int) (getTimeLeft() / 1000)));
        }
    }
    ...
    public void startEvent() {
        ...
        broadcast(EtcPacket.getClock((int) (time / 1000)));
        ...
        msgSchedule = EventTimer.getInstance().register(new Runnable() {
            ...
            public void run() {
                ...
                if (timeLeft > 9000 && timeLeft < 11000) {
                    ...
                    broadcast(MaplePacketCreator.serverNotice(0, "좋아, 당신은 많은 시간이 남아있는이 없습니다. 조금 서둘러주세요!"));
