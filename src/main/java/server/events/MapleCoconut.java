...
public class MapleCoconut extends MapleEvent {
    ...
    public void onMapLoad(MapleCharacter chr) {
        super.onMapLoad(chr);
        chr.getClient().sendPacket(EtcPacket.coconutScore(getCoconutScore()));
    }
    ...
    public void startEvent() {
        ...
        getMap(0).broadcastMessage(EtcPacket.hitCoconut(true, 0, 0));
        getMap(0).broadcastMessage(EtcPacket.getClock(300));
                ...
                if (getMapleScore() == getStoryScore()) {
                    ...
                    for (MapleCharacter chr : getMap(0).getCharactersThreadsafe()) {
                        if (chr.getTeam() == (getMapleScore() > getStoryScore() ? 0 : 1)) {
                            chr.getClient().sendPacket(EtcPacket.showEffect("event/coconut/victory"));
                            chr.getClient().sendPacket(EtcPacket.playSound("Coconut/Victory"));
                        } else {
                            chr.getClient().sendPacket(EtcPacket.showEffect("event/coconut/lose"));
                            chr.getClient().sendPacket(EtcPacket.playSound("Coconut/Failed"));
                        }
                    }
                    warpOut();
                }
    ...
    public void bonusTime() {
        getMap(0).broadcastMessage(EtcPacket.getClock(60));
        EventTimer.getInstance().schedule(new Runnable() {
