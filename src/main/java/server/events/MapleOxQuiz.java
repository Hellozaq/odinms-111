    ...
    public void sendQuestion(final MapleMap toSend) {
        ...
        oxSchedule2 = EventTimer.getInstance().schedule(new Runnable() {
            ...
            public void run() {
                ...
                toSend.broadcastMessage(EtcPacket.showOXQuiz(question.getKey().left, question.getKey().right, true));
                toSend.broadcastMessage(EtcPacket.getClock(10)); //quickly change to 12
            }
        }, 10000);
        if (oxSchedule != null) {
            oxSchedule.cancel(false);
        ...
        oxSchedule = EventTimer.getInstance().schedule(new Runnable() {
            ...
            public void run() {
                ...
                toSend.broadcastMessage(EtcPacket.showOXQuiz(question.getKey().left, question.getKey().right, false));
                timesAsked++;
                for (MapleCharacter chr : toSend.getCharactersThreadsafe()) {
