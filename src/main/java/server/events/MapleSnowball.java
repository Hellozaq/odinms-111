...
public class MapleSnowball extends MapleEvent {
    ...
    public void startEvent() {
        for (int i = 0; i < 2; i++) {
            ...
            getMap(0).broadcastMessage(EtcPacket.enterSnowBall());
        }
    }
    ...
    public static class MapleSnowballs {
        ...
        public void broadcast(MapleMap map, int message) {
            for (MapleCharacter chr : map.getCharactersThreadsafe()) {
                ...
                chr.getClient().sendPacket(EtcPacket.snowballMessage(team, message));
                //}
            }
        }
        ...
        public static final void hitSnowball(final MapleCharacter chr) {
            ...
            if (ball != null && !ball.isInvis()) {
                ...
                if (!snowman) {
                    ...
                    chr.getMap().broadcastMessage(EtcPacket.hitSnowBall(team, damage, 0, 1));
                    if (damage == 0) {
                        if (Math.random() < 0.2) {
                            chr.getClient().sendPacket(EtcPacket.leftKnockBack());
                        ...
                        if (ball.getPosition() == 255 || ball.getPosition() == 511 || ball.getPosition() == 767) { // Going to stage
                            ...
                            chr.getMap().broadcastMessage(EtcPacket.rollSnowball(4, sb.getSnowBall(0), sb.getSnowBall(1)));
                        } else if (ball.getPosition() == 899) { // Crossing the finishing line
                            final MapleMap map = chr.getMap();
                            for (int i = 0; i < 2; i++) {
                                sb.getSnowBall(i).setInvis(true);
                                map.broadcastMessage(EtcPacket.rollSnowball(i + 2, sb.getSnowBall(0), sb.getSnowBall(1))); //inviseble
                            }
                            chr.getMap().broadcastMessage(MaplePacketCreator.serverNotice(6, "축하합니다! " + (team == 0 ? "Story" : "Maple") + "팀이 눈사람굴리기 이벤트에서 승리하였습니다!"));
                    ...
                    chr.getMap().broadcastMessage(EtcPacket.hitSnowBall(team + 2, damage, 0, 0)); // Hitting the snowman
                    ball.setSnowmanHP(ball.getSnowmanHP() - damage);
                    if (damage > 0) {
                        chr.getMap().broadcastMessage(EtcPacket.rollSnowball(0, sb.getSnowBall(0), sb.getSnowBall(1))); //not sure
