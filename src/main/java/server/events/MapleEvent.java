...
public abstract class MapleEvent {
    ...
    public static void givePrize(final MapleCharacter chr) {
        ...
        if (reward == 0) {
            ...
            chr.dropMessage(5, "[通知] 祝賀你，獲得了" + mes + "楓幣的獎勵。");
        } else if (reward == 1) {
            final int cs = Randomizer.nextInt(4000) + 1000;
            chr.modifyCSPoints(1, cs, true);
            chr.dropMessage(5, "[通知] 祝賀你，獲得了" + cs + "楓葉點數的獎勵。");
        } else if (reward == 2) {
            final int cs = Randomizer.nextInt(4000) + 1000;
            chr.modifyCSPoints(1, cs, true);
            chr.dropMessage(5, "[通知] 祝賀你，獲得了" + cs + "楓葉點數的獎勵。");
            ...
            chr.addFame(10);
            chr.dropMessage(5, "[通知] 祝賀你，獲得了10點數的名聲");
        } else if (reward == 4) {
            chr.dropMessage(5, "[通知] 咣~~沒有任何獎勵。");
        } else {
            int max_quantity = 1;
            switch (reward) {
                case 5062000:
            ...
            chr.dropMessage(5, "[通知] 祝賀你，獲得了隨機道具，請注意查收。");
    ...
    public void onMapLoad(MapleCharacter chr) { //most dont do shit here
        if (GameConstants.isEventMap(chr.getMapId()) && FieldLimitType.Event.check(chr.getMap().getFieldLimit()) && FieldLimitType.Event2.check(chr.getMap().getFieldLimit())) {
            chr.getClient().sendPacket(EtcPacket.showEventInstructions());
        }
    }
                ...
                if (e.isRunning) {
                    for (int i : e.type.mapids) {
                        if (cserv.getEvent() == i) {
                            ...
                            //
                            //e.broadcast(MaplePacketCreator.serverNotice(0, "이벤트는 30초후에 시작됩니다."));
                            e.broadcast(EtcPacket.getClock(30));
    ...
    public static final String scheduleEvent(final MapleEventType event, final ChannelServer cserv) {
        ...
        //     World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(0, "안녕하세요!  " + StringUtil.makeEnumHumanReadable(event.name()) + " 이벤트가 " + cserv.getChannel() + "채널에서 시작되려고합니다! " + cserv.getChannel() + "채널로 이동하셔서 @event 를 입력하세요."));
        return "";
    }
}
