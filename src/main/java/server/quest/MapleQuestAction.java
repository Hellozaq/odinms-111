    ...
    public void runStart(MapleCharacter c, Integer extSelection) {
        ...
        switch (type) {
            ...
            case nextQuest:
                ...
                c.getClient().sendPacket(EtcPacket.updateQuestFinish(quest.getId(), status.getNpc(), intStore));
                break;
            case money:
                status = c.getQuest(quest);
                if (status.getForfeited() > 0) {
...
//		MapleQuest.getInstance(intStore).forceComplete(c, 0);
    ...
    public void runEnd(MapleCharacter c, Integer extSelection) {
        switch (type) {
            ...
            case nextQuest: {
                c.getClient().sendPacket(EtcPacket.updateQuestFinish(quest.getId(), c.getQuest(quest).getNpc(), intStore));
                break;
            }
            case money: {
                c.gainMeso(intStore, true, true);
