...
public class MapleReactor extends MapleMapObject {
    ...
    public void sendDestroyData(MapleClient client) {
        client.sendPacket(EtcPacket.destroyReactor(this));
    }

    @Override
    public void sendSpawnData(MapleClient client) {
        client.sendPacket(EtcPacket.spawnReactor(this));
    }
    ...
    public void forceHitReactor(final byte newState) {
        ...
        map.broadcastMessage(EtcPacket.triggerReactor(this, (short) 0));
    }
    ...
    public void hitReactor(int charPos, short stance, MapleClient c) {
        if (c.getPlayer().isGM()) {
            c.getPlayer().dropMessage(6, "[系統提示]您已經建立與反應堆" + rid + "的連接");
        }
            ...
            if (!(stats.getType(state) == 2 && (charPos == 0 || charPos == 2))) { // next state
                ...
                if (stats.getNextState(state) == -1 || stats.getType(state) == 999) { //end of reactor
                    if ((stats.getType(state) < 100 || stats.getType(state) == 999) && delay > 0) { //reactor broken
                        ...
                        map.broadcastMessage(EtcPacket.triggerReactor(this, stance));
                    }
                    //if (rid > 200011) {
                    ReactorScriptManager.getInstance().act(c, this);
                    //}
                    ...
                    boolean done = false;
                    map.broadcastMessage(EtcPacket.triggerReactor(this, stance)); //magatia is weird cause full beaker can be activated by gm hat o.o
