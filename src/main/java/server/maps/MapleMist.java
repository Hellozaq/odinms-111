...
public class MapleMist extends MapleMapObject {
    ...
    public byte[] fakeSpawnData(int level) {
        return EtcPacket.spawnMist(this);
    }

    @Override
    public void sendSpawnData(final MapleClient c) {
        c.sendPacket(EtcPacket.spawnMist(this));
    }

    @Override
    public void sendDestroyData(final MapleClient c) {
        c.sendPacket(EtcPacket.removeMist(getObjectId(), false));
    }
    ...
    public void setSkillDelay(int nSkillDelay) {
        skillDelay = nSkillDelay;
    }
}
