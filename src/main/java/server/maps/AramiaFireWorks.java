...
public class AramiaFireWorks {
    ...
    private final void broadcastServer(final MapleCharacter c, final int itemid) {
        switch (itemid) {
            case SUN_ID: {
                World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, itemid, "잠시 후 단풍나무 언덕의 단풍나무가 터지려 합니다."));
            }
            case DEC_ID: {
                World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, itemid, "잠시 후 행복한마을 행복한나무에서 눈이 내리려합니다!"));
            }
            case KEG_ID: {
                World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(5, itemid, "잠시 후 헤네시스 공원의 아르미가 폭죽을 쏘아 올립니다"));
            }
        }
    }
            ...
            switch (reactor.getState()) {
                ...
                case 4: //first state
                    if (this.sunshines >= (MAX_SUN / 6) * (2 + reactor.getState())) {
                        ...
                        map.broadcastMessage(EtcPacket.triggerReactor(reactor, reactor.getState()));
                    }
                    break;
                default:
    ...
    private final void startSun(final MapleMap map) {
        map.startMapEffect("따듯한 햇살을 받고 나무가 아이템을 터치기 시작합니다!", 5121010);
        for (int i = 0; i < 3; i++) {
            EventTimer.getInstance().schedule(new Runnable() {
    ...
    private final void startDec(final MapleMap map) {
        map.startMapEffect("나무의 신기한 힘 때문에 눈이 내리기 시작하였습니다!", 5120000);
        for (int i = 0; i < 3; i++) {
            EventTimer.getInstance().schedule(new Runnable() {
