...
public class MapleMapItem extends MapleMapObject {
    ...
    public void sendSpawnData(final MapleClient client) {
        if (questid <= 0 || client.getPlayer().getQuestStatus(questid) == 1) {
            client.sendPacket(EtcPacket.dropItemFromMapObject(this, null, getTruePosition(), (byte) 2));
        }
    }

    @Override
    public void sendDestroyData(final MapleClient client) {
        client.sendPacket(EtcPacket.removeItemFromMap(getObjectId(), 1, 0));
    }
    ...
    public void expire(final MapleMap map) {
        pickedUp = true;
        map.broadcastMessage(EtcPacket.removeItemFromMap(getObjectId(), 0, 0));
