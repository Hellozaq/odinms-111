...
public class Event_PyramidSubway {
    ...
    public final void commenceTimerNextMap(final MapleCharacter c, final int stage) {
        ...
        if (c.getParty() != null && c.getParty().getMembers().size() > 1) {
            for (MaplePartyCharacter mpc : c.getParty().getMembers()) {
                ...
                if (chr != null) {
                    chr.getClient().sendPacket(EtcPacket.getClock(time));
                    chr.getClient().sendPacket(EtcPacket.showEffect("killing/first/number/" + stage));
                    chr.getClient().sendPacket(EtcPacket.showEffect("killing/first/stage"));
                    chr.getClient().sendPacket(EtcPacket.showEffect("killing/first/start"));
                    fullUpdate(chr, stage);
                }
            }
        } else {
            c.getClient().sendPacket(EtcPacket.getClock(time));
            c.getClient().sendPacket(EtcPacket.showEffect("killing/first/number/" + stage));
            c.getClient().sendPacket(EtcPacket.showEffect("killing/first/stage"));
            c.getClient().sendPacket(EtcPacket.showEffect("killing/first/start"));
            fullUpdate(c, stage);
        }
    ...
    public final void succeed(final MapleCharacter c) {
        ...
        c.getClient().sendPacket(EtcPacket.showEffect("killing/clear"));
        c.getClient().sendPacket(EtcPacket.sendPyramidResult(rank, exp));
        dispose(c);
    }
    ...
    public final void broadcastUpdate(final MapleCharacter c) {
        c.getClient().sendPacket(EtcPacket.sendPyramidUpdate(bar));
    }

    public final void broadcastEffect(final MapleCharacter c, final String effect) {
        c.getClient().sendPacket(EtcPacket.showEffect(effect));
    }
    ...
    private static final void changeMap(final MapleCharacter c, final MapleMap map, final int minLevel, final int maxLevel, final int clear) {
        ...
        if (c.getParty() != null && c.getParty().getMembers().size() > 1) {
            for (MaplePartyCharacter mpc : c.getParty().getMembers()) {
                ...
                if (chr != null && chr.getId() != c.getId() && chr.getLevel() >= minLevel && chr.getLevel() <= maxLevel) {
                    if (clear == 1) {
                        chr.getClient().sendPacket(EtcPacket.showEffect("killing/clear"));
                    } else if (clear == 2) {
                        chr.getClient().sendPacket(EtcPacket.showEffect("killing/fail"));
                    }
        ...
        if (clear == 1) {
            ...
            c.getClient().sendPacket(EtcPacket.showEffect("killing/fail"));
        }
