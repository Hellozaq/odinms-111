...
public class MapleMapFactory {
    ...
    public EventInstanceManager getInstanceManager() {
        return eim;
    }

    public void setInstanceManager(EventInstanceManager d) {
        eim = d;
    }
                    ...
                    try {
                        ...
                        while (rs.next()) {
                            ...
                            if (npc != null) {
                                ...
                                System.err.println("Npc Data Error");
                            }
                        }
                    } catch (Exception e) {
                        System.err.println("Npc Dataload Error");
                    } finally {
                        if (ps != null) {
                            try {
                                ps.close();
...
//			System.err.println("Hide > 1 ("+ hide +")");
    ...
    public MapleMap getMapMpk(int mapid) {
        MapleMap map = (MapleMap) getMap(mapid);
        MapleData mapData = source.getData(getMapName(mapid));
        for (MapleData life : mapData.getChildByPath("life")) {
            String id = MapleDataTool.getString(life.getChildByPath("id"));
            String type = MapleDataTool.getString(life.getChildByPath("type"));
            AbstractLoadedMapleLife Life = loadLife(life, id, type);
