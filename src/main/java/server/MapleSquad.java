...
public class MapleSquad {
    ...
    public static enum MapleSquadType {
        bossbalrog(2), zak(2), chaoszak(3), horntail(2), chaosht(3), pinkbean(3), nmm_squad(2), vergamot(2), dunas(2), nibergen_squad(2), dunas2(2), core_blaze(2), aufheben(2), cwkpq(10), tokyo_2095(2), vonleon(3), scartar(2), cygnus(3), arkarium(3), showa(3);
            ...
            if (theirCh > 0) {
                ...
                if (lead != null && lead.getMapId() == beginMapId && lead.getClient().getChannel() == ch) {
                    ...
                    if (ChannelServer.getInstance(ch).addMapleSquad(squad, type.name())) {
                        getBeginMap().broadcastMessage(EtcPacket.getClock(expiration / 1000));
    ...
    public void clear() {
        if (removal != null) {
            getBeginMap().broadcastMessage(EtcPacket.stopClock());
