...
public class MapleAchievement {
    ...
    public void finishAchievement(MapleCharacter chr) {
        ...
        if (notice && !chr.isGM()) {
            World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "[通知] 獲得了" + (GameConstants.GMS ? (reward / 2) : reward) + "楓葉點數的獎勵。"));
        } else {
            chr.getClient().sendPacket(MaplePacketCreator.serverNotice(5, "[通知] 獲得了" + (GameConstants.GMS ? (reward / 2) : reward) + "楓葉點數的獎勵。"));
        }
    }
}
