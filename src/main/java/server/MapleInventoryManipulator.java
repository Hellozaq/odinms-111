...
public class MapleInventoryManipulator {
    ...
    public static boolean addById(MapleClient c, int itemId, short quantity, String gmLog, boolean p, long period) {
        return addById(c, itemId, quantity, null, null, period, gmLog, p);
    }
    ...
    public static boolean addById(MapleClient c, int itemId, short quantity, String owner, MaplePet pet, long period, String gmLog, boolean p) {
        return addId(c, itemId, quantity, owner, pet, period, gmLog, p) >= 0;
    }

    public static byte addId(MapleClient c, int itemId, short quantity, String owner, MaplePet pet, long period, String gmLog, boolean p) {
        ...
        if (!type.equals(MapleInventoryType.EQUIP)) {
            ...
            if (!GameConstants.isRechargable(itemId)) {
                ...
                while (quantity > 0) {
                    ...
                    if (newQ != 0) {
                        ...
                        if (period > 0) {
                            if (itemId == 4001432) {//高級黃金券
                                nItem.setExpiration(System.currentTimeMillis() + (1 * 60 * 60 * 1000));//1小時限定
                            } else {
            ...
            if (quantity == 1) {
                ...
                if (p) {
                    ((Equip) nEquip).renewPotential(2);//구매시 에픽등급 지정
                }
                if (period > 0) {
    ...
    public static boolean addFromDrop(final MapleClient c, Item item, final boolean show, final boolean enhance, final boolean hm) {
        final MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
        ...
        if (!type.equals(MapleInventoryType.EQUIP)) {
            ...
            if (!GameConstants.isRechargable(item.getItemId())) {
                    ...
                    while (quantity > 0) {
                        if (i.hasNext()) {
                            ...
                            if (oldQ < slotMax && item.getOwner().equals(eItem.getOwner()) && item.getExpiration() == eItem.getExpiration()) {
                                ...
                                quantity -= (newQ - oldQ);
                                eItem.setQuantity(newQ);
                                c.sendPacket(InventoryPacket.updateInventorySlot(type, eItem, hm));
                            }
                        } else {
                            break;
                        }
                ...
                while (quantity > 0) {
                    ...
                    c.sendPacket(InventoryPacket.addInventorySlot(type, nItem, hm));
                }
            } else {
                // Throwing Stars and Bullets - Add all into one slot regardless of quantity.
            ...
            if (quantity == 1) {
                ...
                c.sendPacket(InventoryPacket.addInventorySlot(type, item, hm));
                if (GameConstants.isHarvesting(item.getItemId())) {
                    c.getPlayer().getStat().handleProfessionTool(c.getPlayer());
                }
            } else {
    ...
    public static void equip(final MapleClient c, final short src, short dst) {
        ...
        if (source.getItemId() / 10000 == 166) {
            ...
            if (target2 == null) {
                c.getPlayer().dropMessage(1, "需要安裝安卓心臟道具才能啟動。");
                c.sendPacket(MaplePacketCreator.enableActions());
                return;
            }
        ...
        } else if (source.getItemId() == 1122017 || source.getItemId() == 1122156) {
            chr.equipPendantOfSpirit();
        } else if (source.getItemId() == 1122121) { //自动拾取
            if (chr.getAutoLoot() == false) {
                chr.setAutoLoot(true);
            }
            chr.dropMessage(6, "穿戴特殊装备，可以激活其属性。");
        }
            ...
            for (int i : potentials) {
                if (i > 0) {
                    ...
                    if (pot != null && pot.get("skillID") > 0) {
                        chr.dropMessage(5, SkillFactory.getSkillName(pot.get("skillID")) + " 技能已啟動。");
    ...
    public static void unequip(final MapleClient c, final short src, final short dst) {
        ...
        if (source.getAndroid2()) {
            Equip target2 = (Equip) c.getPlayer().getInventory(MapleInventoryType.EQUIPPED).getItem((short) -53);
            if (target2 != null) {
                c.getPlayer().dropMessage(1, "請先將安卓部件取下來。");
                c.sendPacket(MaplePacketCreator.enableActions());
                return;
            }
        }
        ...
        if (source.getState() >= 17) {
            ...
            for (int i : potentials) {
                if (i > 0) {
                    StructItemOption pot = ii.getPotentialInfo(i).get(ii.getReqLevel(source.getItemId()) / 10);
                    if (pot != null && pot.get("skillID") > 0) {
                        c.getPlayer().dropMessage(5, SkillFactory.getSkillName(pot.get("skillID")) + " 技能被禁止使用。");
