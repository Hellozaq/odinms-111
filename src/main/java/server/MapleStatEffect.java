...
public class MapleStatEffect implements Serializable {
    ...
    private short hp, mp, watk, matk, wdef, mdef, acc, avoid, hands, speed, jump, psdSpeed, psdJump, mpCon, hpCon, forceCon, bdR, damage, prop,
            ...
            asrR, terR, er, damR, padX, madX, mesoR, thaw, selfDestruction, PVPdamage, indiePad, indieMad, emad, fatigueChange,
    ...
    public static final MapleStatEffect loadItemEffectFromData(final MapleData source, final int itemid, final int level) {
        return loadFromData(source, itemid, false, false, level, null);
    }
    ...
    private static MapleStatEffect loadFromData(final MapleData source, final int sourceid, final boolean skill, final boolean overTime, final int level, final String variables) {
        ...
        if (ret.overTime && ret.getSummonMovementType() == null && !ret.isEnergyCharge()) {
            ...
            if (sourceid != 22131001) {//龍魔導士這個技能是被動加的HP上限
            ...
            addBuffStatPairToListIfNotZero(ret.statups, MapleBuffStat.ANGEL_MATK, Integer.valueOf(ret.emad));
        ...
        if (sourceid == 2022125) { //비홀더스 5종 버프 구간
            ret.statups.put(MapleBuffStat.WDEF, Integer.valueOf(25 * level));
            ret.overTime = true;
            ret.duration = ((20 + 4 * level) * 1000);
        } else if (sourceid == 2022126) {
            ret.statups.put(MapleBuffStat.MDEF, Integer.valueOf(25 * level));
            ret.overTime = true;
            ret.duration = ((20 + 4 * level) * 1000);
        } else if (sourceid == 2022127) {
            ret.statups.put(MapleBuffStat.ACC, Integer.valueOf(15 * level));
            ret.overTime = true;
            ret.duration = ((20 + 4 * level) * 1000);
        } else if (sourceid == 2022128) {
            ret.statups.put(MapleBuffStat.AVOID, Integer.valueOf(15 * level));
            ret.overTime = true;
            ret.duration = ((20 + 4 * level) * 1000);
        } else if (sourceid == 2022129) {
            ret.statups.put(MapleBuffStat.WATK, Integer.valueOf(5 + 3 * (level / 2)));
            ret.overTime = true;
            ret.duration = ((20 + 4 * level) * 1000);
        } //비홀더스 버프 구간 종료
        if (ret.skill) { // hack because we can't get from the datafile...
            switch (sourceid) {
                ...
                case 23111000://斯特萊克雙擊
                    ret.monsterStatus.put(MonsterStatus.STUN, Integer.valueOf(1));
                    break;
                case 22181000: // idk if this fix, whatever玛瑙的祝福
                ...
                case 13111005: //알바트로스
                    ret.statups.clear();
                    ret.statups.put(MapleBuffStat.SPEED, (int) ret.speed);
                    ret.statups.put(MapleBuffStat.JUMP, (int) ret.jump);
                    ret.statups.put(MapleBuffStat.ENHANCED_MDEF, (int) ret.emdef);
                    ret.statups.put(MapleBuffStat.ENHANCED_WDEF, (int) ret.ewdef);
                ...
                case 2221001:
                    ...
                    ret.duration *= 1; // freezing skills are a little strange
                    break;
                ...
                case 35111009:
                    ret.duration = 2100000000;
                    ret.statups.put(MapleBuffStat.PUPPET, 1); // summon
                    break;
                case 4341006:
                case 3120012:
                case 3220012:
    ...
    public final boolean applyTo(final MapleCharacter applyfrom, final MapleCharacter applyto, final boolean primary, final Point pos, int newDuration) {
        ...
        if (isDispel() && makeChanceResult()) {
            ...
            for (MapleMist m : applyto.getMap().getAllMistsThreadsafe()) {
                applyto.getMap().broadcastMessage(EtcPacket.removeMist(m.getObjectId(), true));
        ...
        if (expinc != 0) {
            ...
            if (mobid > 0) {
                ...
                applyto.getClient().sendPacket(MaplePacketCreator.getCard(done ? sourceid : 0, 1));
            }
            ...
            applyto.getClient().sendPacket(EtcPacket.getPVPScore(x + bs, false));
            ...
            applyto.getClient().sendPacket(EtcPacket.getPVPIceGage(x - iceGageCon));
            ...
            applyto.getClient().sendPacket(EtcPacket.rechargeCombo(applyto.getCombo()));
            ...
            if (portal != null) {
                applyto.getClient().sendPacket(EtcPacket.dragonBlink(portal.getId()));
        ...
        if (summonMovementType != null && (sourceid != 32111006 || (applyfrom.getBuffedValue(MapleBuffStat.REAPER) != null && !primary)) && !applyto.isClone()) {
            ...
            if (isBeholder()) {
                ...
                applyfrom.getClient().sendPacket(EtcPacket.skillCooldown(sourceid, getCooldown(applyfrom)));
                applyfrom.addCooldown(sourceid, System.currentTimeMillis(), getCooldown(applyfrom) * 1000);
                applyfrom.getMap().broadcastMessage(EtcPacket.teslaTriangle(applyfrom.getId(), count.get(0), count.get(1), count.get(2)));
            } else if (sourceid == 35121003) {
                applyfrom.getClient().sendPacket(MaplePacketCreator.enableActions()); //doubt we need this at all
            }
        } else if (isMechDoor()) {
            int newId = 0;
            ...
            if (applyto.getMechDoors().size() >= 2) {
                ...
                newId = remove.getId();
                applyto.getMap().broadcastMessage(EtcPacket.removeMechDoor(remove, true));
            ...
            applyto.getClient().sendPacket(MaplePacketCreator.mechPortal(door.getTruePosition()));
            if (!applyBuff) {
        ...
        if (isMagicDoor()) { // Magic Door
            ...
            for (MapleCoolDownValueHolder i : applyto.getCooldowns()) {
                if (i.skillId != 5121010) {
                    ...
                    applyto.getClient().sendPacket(EtcPacket.skillCooldown(i.skillId, 0));
                }
            }
        } else {
                    ...
                    if (isTimeLeap()) {
                        for (MapleCoolDownValueHolder i : affected.getCooldowns()) {
                            if (i.skillId != 5121010) {
                                ...
                                affected.getClient().sendPacket(EtcPacket.skillCooldown(i.skillId, 0));
                            }
                        }
                    }
                }
        ...
        switch (sourceid) {
            ...
            case 5320007: {//dice
                ...
                if (doublediceid > 0) {
                    ...
                    applyto.getClient().sendPacket(EffectPacket.showOwnDiceEffect(sourceid, rand1, rand2 > 0 ? -1 : 0, level));
                    if (rand1 == 1 && rand2 == 1) {
                        applyto.dropMessage(5, "幸運骰子點數 [" + rand2 + "], [" + rand1 + "] 沒有收到任何效果。");
                        return;
                    } else if (rand1 == 1) {
                        applyto.dropMessage(5, "幸運骰子點數 [" + rand2 + "] 獲得隨機的加持。");
                    } else if (rand2 == 1) {
                        applyto.dropMessage(5, "幸運骰子點數 [" + rand1 + "] 獲得隨機的加持。");
                    } else {
                        applyto.dropMessage(5, "幸運骰子點數 [" + rand2 + "], [" + rand1 + "] 獲得隨機的加持。");
                    }
                    ...
                    applyto.getClient().sendPacket(BuffPacket.giveDice(rand1, sourceid, localDuration, localstatups));
                    applyto.getClient().sendPacket(BuffPacket.giveDice(rand2, sourceid, localDuration, localstatups));
                    showEffect = false;
                } else {
                    ...
                    applyto.getClient().sendPacket(EffectPacket.showOwnDiceEffect(tempsource, diceid, -1, level));
                    if (diceid <= 1) {
                        applyto.dropMessage(5, "幸運骰子點數 [1] 沒有收到任何效果。");
                        return;
                    }
                    applyto.dropMessage(5, "幸運骰子點數 [" + diceid + "] 獲得隨機的加持。");
                    localstatups = new EnumMap<MapleBuffStat, Integer>(MapleBuffStat.class);
                    localstatups.put(MapleBuffStat.DICE_ROLL, diceid);
                    applyto.getClient().sendPacket(BuffPacket.giveDice(diceid, tempsource, localDuration, localstatups));
                    showEffect = false;
                }
                normal = false;
                break;
            ...
            case 5311004: {
                ...
                applyto.getClient().sendPacket(EffectPacket.showOwnDiceEffect(sourceid, zz, -1, (int) level));
                localstatups = new EnumMap<MapleBuffStat, Integer>(MapleBuffStat.class);
            ...
            case 13101006: { // Wind Walk
                ...
                final EnumMap<MapleBuffStat, Integer> stat = new EnumMap<MapleBuffStat, Integer>(MapleBuffStat.class);
                stat.put(MapleBuffStat.WIND_WALK, 0); // HACK..
                applyto.getMap().broadcastMessage(applyto, BuffPacket.giveForeignBuff(applyto.getId(), stat, this), false);
                break;
            }
            case 4001003: {
            ...
            case 32111005: { //超級體
                ...
                if (applyfrom.getStatForBuff(MapleBuffStat.BLUE_AURA) != null) {
                    ...
                    applyto.getClient().sendPacket(BuffPacket.giveBuff(sourcez, ilocalDuration, istat, this));
                    final long starttime = System.currentTimeMillis();
            ...
            case 32001003: {// 黑色繩索
                ...
                applyto.getClient().sendPacket(BuffPacket.giveBuff(32001003, localDuration, statt, this));
            ...
            case 32111012: { // 藍色繩索
                ...
                applyto.getClient().sendPacket(BuffPacket.giveBuff(32111012, localDuration, statt, this));
                applyto.getMap().broadcastMessage(applyto, BuffPacket.giveForeignBuff(applyto.getId(), statt, this), false);
                break;
            }
            case 32110008: {
            ...
            case 32101003: { // 黃色繩索
                ...
                applyto.getClient().sendPacket(BuffPacket.giveBuff(32101003, localDuration, statt, this));
                applyto.getMap().broadcastMessage(applyto, BuffPacket.giveForeignBuff(applyto.getId(), statt, this), false);
                break;
            }
            case 32110009:
            ...
            case 15111006: { // Spark
                localstatups = new EnumMap<MapleBuffStat, Integer>(MapleBuffStat.class);
                localstatups.put(MapleBuffStat.SPARK, (int) damage);
                applyto.getClient().sendPacket(BuffPacket.giveBuff(sourceid, localDuration, localstatups, this));
                normal = false;
                break;
            }
            ...
            case 31121005: { // Dark Metamorphosis
                ...
                final EnumMap<MapleBuffStat, Integer> stat = new EnumMap<MapleBuffStat, Integer>(MapleBuffStat.class);
                stat.put(MapleBuffStat.DARK_METAMORPHOSIS, 6); // 최대 몹 카운터
                applyto.getMap().broadcastMessage(applyto, BuffPacket.giveForeignBuff(applyto.getId(), stat, this), false);
                break;
            }
            case 2121004:
           ...
           case 180:
                duration = 2100000000;
                statups.put(MapleBuffStat.ANGEL_ATK, x);
                statups.put(MapleBuffStat.ANGEL_MATK, x);
            ...
            */
            ...
            default:
                if (isPirateMorph()) {
                    ...
                    applyto.getMap().broadcastMessage(applyto, BuffPacket.giveForeignBuff(applyto.getId(), stat, this), false);
                    applyto.getClient().sendPacket(BuffPacket.giveBuff(sourceid, localDuration, stat, this));
                    //System.out.print(applyto.getId()+" / "+applyfrom.getId());
                    ...
                    normal = maskedStatups.size() > 0; //坐骑属性控制（会多生成一个BUFF图标，用于给角色增加能力值。）
                } else if (isSoaring()) {
                    if (applyto.isHidden()) {
                        break;
                    }
