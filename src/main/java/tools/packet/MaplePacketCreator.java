...
import constants.DojoRankEntry;
...
public class MaplePacketCreator {
    ...
    public static class BuffPacket {
        ...
        public static byte[] cancelArcainAim() {
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static class InfoPacket {
        public static byte[] getDojoRanking(List<DojoRankEntry> ranking) {
            final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();

            mplew.writeShort(SendPacketOpcode.DOJO_RANKING.getValue());
            mplew.writeInt(ranking.size());//SIZE
            int i = 1;
            for (DojoRankEntry rank : ranking) {
                mplew.writeShort(i);
                mplew.writeMapleAsciiString(rank.getName());
                mplew.writeLong(rank.getTime());
                i++;
            }
            return mplew.getPacket();
        }

        public static byte[] showkareta() {
            final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
            mplew.writeShort(SendPacketOpcode.SHOW_STATUS_INFO.getValue());
            mplew.write(12);
            mplew.write(HexTool.getByteArrayFromHexString("68 1E 04 00 73 77 3D 31"));
            return mplew.getPacket();
        }

        public static byte[] showkaretaeffect() {
            final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
            mplew.writeShort(SendPacketOpcode.SHOW_ITEM_GAIN_INCHAT.getValue());
            mplew.write(0x20);
            mplew.writeMapleAsciiString("Effect/BasicEff.img/JobChangedElf");
            mplew.write(1);
            mplew.writeInt(0);
            mplew.writeInt(2);
            mplew.writeInt(5155000);
            return mplew.getPacket();
        }
        ...
        public static byte[] GainEXP_Monster(final int gain, final boolean white, final int partyinc, final int Class_Bonus_EXP, final int Equipment_Bonus_EXP, final int Premium_Bonus_EXP, final int ElfBonus) {
            ...
            mplew.writeInt(Premium_Bonus_EXP); // PCroom_Bonus_EXP (+%d)
            mplew.writeInt(0); // RainBowWeek_Bonus_EXP (+%d)
            mplew.write(0); // Monster Card Completion Set +exp
            mplew.writeInt(0); // Boom Up Bonus EXP (+%d)
            mplew.writeInt(0); // Potion Bonus EXP (+%d)
            mplew.writeInt(ElfBonus/*20021110*/); // %s Bonus EXP (+%d) (string bonus exp?)
    ...
    public static class GuildPacket {
        ...
        public static byte[] guildCapacityChange(int gid, int capacity) {
            // 한글화불가, 늘려서 축하한다  뭐 대충이런소리임 대충알아들어라
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static byte[] charInfo(final MapleCharacter chr, final boolean isSelf) {
        ...
        mplew.write(chr.getProfessionsnum());
        for (int i = 1; i < (chr.getProfessionsnum() + 1); i++) {
            mplew.writeShort(chr.getProfessionsid(i));
        }
        if (chr.getGuildId() <= 0) {
            mplew.writeMapleAsciiString("-");
            mplew.writeMapleAsciiString("");
    ...
    public static byte[] serverNotice(String message, int id) {
        return serverMessage(6, 0, message, false, id);
    }
