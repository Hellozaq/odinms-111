...
public class PacketHelper {
    ...
    public static final void addSkillInfo(final MaplePacketLittleEndianWriter mplew, final MapleCharacter chr) { // 0x100
        ...
        for (Entry<Skill, SkillEntry> skill : skills.entrySet()) {
            ...
            addExpirationTime(mplew, skill.getValue().expiration); //上線載入技能顯示的時間
    ...
    public static final void addCharStats(final MaplePacketLittleEndianWriter mplew, final MapleCharacter chr) {
        ...
        for (int x = chr.getName().getBytes().length; x < 13; x++) {
            mplew.write(0);
        }
    ...
    public static final void addCharLook(final MaplePacketLittleEndianWriter mplew, final MapleCharacterLook chr, final boolean mega) {
        ...
        mplew.write(chr.isElf() ? 1 : 0);//GameConstants.isMercedes(chr.getJob()) ? 1 : 0
    ...
    public static final void addItemInfo(final MaplePacketLittleEndianWriter mplew, final Item item, final MapleCharacter chr) {
        ...
        if (item.getPet() != null) { // Pet
            ...
            if (item.getType() == 1) {
                ...
                mplew.writeMapleAsciiString(equip.getOwner());//이름
                mplew.writeShort(equip.getFlag());//플래그
                ...
                mplew.writeInt(equip.getDurability());//내구도
                mplew.writeInt(equip.getViciousHammer());//황망
                mplew.writeShort(equip.getPVPDamage());//pvp공
                mplew.write(equip.getState()); // 17 = rare, 18 = epic, 19 = unique, 20 = legendary, potential flags.
                mplew.write(equip.getEnhance());//성
                mplew.writeShort(equip.getPotential1());//선두
                if (!hasUniqueId) {
                    mplew.writeShort(equip.getPotential2());//후미
                    mplew.writeShort(equip.getPotential3());
                    mplew.writeShort(equip.getPotential4());
                    mplew.writeShort(equip.getPotential5());
                }
                // if(equip.getSocketState() != SocketFlag.DEFAULT.getValue()){
                mplew.writeShort(equip.getSocketState() != -1 ? equip.getSocketState() : -1);//소켓
                mplew.writeShort(equip.getSocket1() != -1 ? equip.getSocket1() % 10000 : -1); // > 0 = mounted, 0 = empty, -1 = none.
                mplew.writeShort(equip.getSocket2() != -1 ? equip.getSocket2() % 10000 : -1);
                mplew.writeShort(equip.getSocket3() != -1 ? equip.getSocket3() % 10000 : -1);
                /*//}else{
                mplew.writeShort(-1);//소켓
                mplew.writeShort(-1); // > 0 = mounted, 0 = empty, -1 = none.
                mplew.writeShort(-1);
                mplew.writeShort(-1);
                }*/
                //mplew.writeShort(-1);//소켓
    ...
    public static void addOmok(final MaplePacketLittleEndianWriter mplew, IMaplePlayerShop shop, MapleCharacter owner) {
        ...
        mplew.write(shop.getGameType() == 1 ? 0 : shop.getPieceType());
    ...
    public static final void addShopItemInfo(final MaplePacketLittleEndianWriter mplew, final MapleShopItem item, final MapleShop shop, final MapleItemInformationProvider ii, final Item i) {
        ...
        mplew.writeInt(item.getPeriod()); //Period  1분 = 1, 1일 = 1440
        mplew.writeInt(0); // 최소레벨? 이건모르겟
        mplew.writeInt(item.getTab()); //탭 7 = 스페셜
        mplew.write(item.getPotential() == 1 ? 1 : 0); // 잠재설정
        mplew.writeInt(0);
