...
public class LoginPacket {
    ...
    public static final byte[] getAuthSuccessRequest(final MapleClient client) throws SQLException {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        ...
        if (ServerConstants.ENABLE_PIC) { //pic
            if (!client.getPicOF()) {
                mplew.write(2);
            } else {
                mplew.write(client.getSecondPassword() == null ? 0 : (client.getSecondPassword().equals("") ? 2 : 1)); //2 = no pic at all   // 0 = Register PIC, 1 = Ask for PIC, 2 = Disabled
            }
        } else {
            mplew.write(2);
        }
    ...
    public static final byte[] getLoginWelcome() {
        List<Pair<String, Integer>> flags = new LinkedList<Pair<String, Integer>>();
        flags.add(new Pair<String, Integer>("20120111", 0));
        flags.add(new Pair<String, Integer>("returnLegend2", 0));
        return EtcPacket.spawnFlags(flags);
    }
    ...
    public static final byte[] getCharList(final MapleClient client, final List<MapleCharacter> chars, int charslots) throws SQLException {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        ...
        if (ServerConstants.ENABLE_PIC) { //pic
            if (client.getSecondPassword() == null) {
                mplew.write(0);
            } else if (!client.getPicOF()) {
                mplew.write(2);
            } else {
                mplew.write(1);
                //mplew.write(client.getSecondPassword() == null ? 0 : (client.getSecondPassword().equals("") ? 2 : 1)); //2 = no pic at all   // 0 = Register PIC, 1 = Ask for PIC, 2 = Disabled    
    ...
    public static byte[] showAllCharacterInfo(int worldid, List<MapleCharacter> chars, String pic, Boolean usepic) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        mplew.writeShort(SendPacketOpcode.ALL_CHARLIST.getValue());
    ...
    public static byte[] enableSpecialCreation(int accid, boolean enable) {
        ...
        if (ServerConstants.DISABLE_SPEACIAL_CREATION) { //特殊职业创建
            mplew.write(enable ? 0 : 1);
        } else {
            mplew.write(1);
        }
