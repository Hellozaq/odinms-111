...
public class Skill {
    ...
    private int id, animationTime = 0, masterLevel = 0, maxLevel = 0, delay = 0, trueMax = 0, eventTamingMob = 0, skillTamingMob = 0, skillType = 0, x; //4 is alert
    private boolean invisible = true, chargeskill = false, timeLimited = false, combatOrders = false, pvpDisabled = false, magic = false, casterMove = false, pushTarget = false, pullTarget = false;
    ...
    public static final Skill loadFromData(final int id, final MapleData data, final MapleData delayData) {
        ...
        ret.x = MapleDataTool.getInt("x", data, 0);
        if ((id == 22111001 || id == 22140000 || id == 22141002)) {
            ret.masterLevel = 20; //hack 
        }
