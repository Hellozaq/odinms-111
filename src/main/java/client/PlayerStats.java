...
public class PlayerStats implements Serializable {
    ...
    public void setSP(int i, int d) {
        this.remainingSp[i] += d;
    }
    ...
    public void recalcLocalStats(boolean first_login, MapleCharacter chra) {
        ...
        while (itera.hasNext()) {
            ...
            if (equip.getItemId() >= 1082392 && equip.getItemId() <= 1082394) { //영웅장갑
        ...
        switch (chra.getJob()) {
            ...
            case 3212:
                ...
                bx = SkillFactory.getSkill(32121004);//黑暗世紀
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    eff = bx.getEffect(bof);
                    damageIncrease.put(32111003, (int) bx.getEffect(bof).getDAMRate());
                }
                bx = SkillFactory.getSkill(32111002);//絕命攻擊
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    eff = bx.getEffect(bof);
                    ignoreTargetDEF += eff.getIgnoreMob();
                }
                bx = SkillFactory.getSkill(32121002);//終極攻擊
                bof = chra.getTotalSkillLevel(bx);
                if (bof > 0) {
                    eff = bx.getEffect(bof);
                    ignoreTargetDEF += eff.getIgnoreMob();
