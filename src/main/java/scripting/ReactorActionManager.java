...
public class ReactorActionManager extends AbstractPlayerInteraction {
    ...
    public void cancelHarvest(boolean succ) {
        ...
        getPlayer().getMap().broadcastMessage(getPlayer(), EtcPacket.cancelHarvesting(getPlayer().getClient()), false);
        getPlayer().getMap().broadcastMessage(EtcPacket.harvestResult(getPlayer().getId(), succ));
    }
    ...
    public void doHarvest() { //TODO LEGEND
        ...
        cancelHarvest(succ);
        Gmtext(7, pName + "的熟練度提高了。 (+" + masteryIncrease + ")");
        if (getPlayer().addProfessionExp(pID, masteryIncrease)) {
            Gmtext(7, pName + " 的等級提升了。");
        }
    }
}
