...
public class NPCConversationManager extends AbstractPlayerInteraction {
    ...
    public void sendGetNumberS(int type, int type2, String text, int def, int min, int max) {
        ...
        c.sendPacket(NPCPacket.getNPCTalkNumS(type, type2, id, text, def, min, max));
        lastMsg = 4;
    }
    ...
    public void sendStorage() {
        if (c.getPlayer().getLevel() < 10) {
            c.getPlayer().dropMessage(1, "等級10以下的玩家還不能使用倉庫。");
        } else {
    ...
    public void showEffect(boolean broadcast, String effect) {
        if (broadcast) {
            ...
            c.sendPacket(EtcPacket.showEffect(effect));
        }
    }
    ...
    public void playSound(boolean broadcast, String sound) {
        if (broadcast) {
            ...
            c.sendPacket(EtcPacket.playSound(sound));
        }
    }
    ...
    public void environmentChange(boolean broadcast, String env) {
        if (broadcast) {
            ...
            c.sendPacket(EtcPacket.environmentChange(env, 2));
        }
    }
    ...
    public void increaseGuildCapacity(boolean trueMax) {
        if (c.getPlayer().getMeso() < 500000 && !trueMax) {
            c.sendPacket(MaplePacketCreator.serverNotice(1, "你沒有足夠的楓幣。"));
            return;
        }
        final int gid = c.getPlayer().getGuildId();
        if (gid <= 0) {
        ...
        if (World.Guild.increaseGuildCapacity(gid, trueMax)) {
            ...
            sendNext("請檢查你的公會人數是否達到上限(限制: 100)");
        } else {
            sendNext("請檢查你的公會人數是否達到上限，或者沒有足夠的25000的GP。(Limit: 200)");
        }
    }
    ...
    public void openDuey() {
        ...
        c.sendPacket(EtcPacket.sendDuey((byte) 8, null, null));
    }
    ...
    public void sendPVPWindow() {
        ...
        c.sendPacket(EtcPacket.sendPVPMaps());
    }

    public void sendLinkSkillWindow(int skillId) {//連接技能視窗
        if (hasSkill(skillId)) {
            c.sendPacket(UIPacket.sendLinkSkillWindow(skillId));
        }
    }

    public void sendPartyWindow() {//組隊搜索視窗
        c.sendPacket(UIPacket.sendPartyWindow(id));
    }
    ...
    public void sendProfessionWindow() {//專業技術視窗
        c.sendPacket(UIPacket.sendProfessionWindow(0));
    }

    public void sendEventWindow() {//活動清單視窗
        c.sendPacket(UIPacket.sendEventWindow(0));
    }
    ...
    public final void sendRPS() {
        c.sendPacket(EtcPacket.getRPSMode((byte) 8, -1, -1, -1));
    }
    ...
    public final void TimeMoveMap(final int destination, final int movemap, final int time) {
        warp(movemap, 0);
        getClient().sendPacket(EtcPacket.getClock(time));
    ...
    public final void doWeddingEffect(final Object ch) {
        ...
        getMap().broadcastMessage(MaplePacketCreator.yellowChat(player.getName() + " 妳願意承認 " + chr.getName() + " 做妳的丈夫，誠實遵照上帝的誡命，和他生活在一起，無論在什麼環境願順服他、愛惜他、安慰他、尊重他保護他，以致奉召歸主？"));
        CloneTimer.getInstance().schedule(new Runnable() {
            ...
            public void run() {
                if (chr == null || player == null) {
                    ...
                    chr.getMap().broadcastMessage(MaplePacketCreator.yellowChat(chr.getName() + " 你願意承認接納 " + player.getName() + " 做你的妻子，誠實遵照上帝的誡命，和她生活在一起，無論在什麼環境，願意終生養她、愛惜她、安慰她、尊重她、保護她，以至奉召歸主？"));
                }
            }
        }, 10000);
        CloneTimer.getInstance().schedule(new Runnable() {
            ...
            public void run() {
                if (chr == null || player == null) {
                    ...
                    sendNPCText(player.getName() + " 和 " + chr.getName() + ", 從現在開始 " + chr.getClient().getChannelServer().getServerName() + " 我希望你們兩個能在此時此刻永遠愛著對方！", 9201002);
                    chr.getMap().startExtendedMapEffect("那麼現在請新娘親吻 " + getPlayer().getName() + "！", 5120006);
                    if (chr.getGuildId() > 0) {
    ...
    public void putKey(int key, int type, int action) {
        ...
        getClient().sendPacket(EtcPacket.getKeymap(getPlayer().getKeyLayout()));
    }
    ...
    public String getJobRanking(int job) {
        String text = "#b:: " + (job == -1 ? "全部" : GameConstants.getJobName(job)) + "職業排名 ::#k\r\n\r\n";
    ...
    public void sendPendant(boolean b) {
        c.sendPacket(MaplePacketCreator.pendantSlot(b));
    }
    ...
    public void manageItem(String type, int item, int quality) {
        ...
        if (squad != null) {
            ...
            c.getPlayer().dropMessage(-1, "管理功能僅對當前連接的用戶可用。");
        }
    }
    ...
    public void showDojoRanking() {
        c.sendPacket(InfoPacket.getDojoRanking(DojoRank.getDojoRank()));
    }
    
    public static int chackOF(int accid, int type) throws SQLException {
        int cfg = 1; //0关 1开
        Connection con = DatabaseConnection.getConnection();
        PreparedStatement ps = null;
        ps = con.prepareStatement("SELECT cfg FROM playercfg WHERE accid = ? and type = ?");
    ...
    public void 设置开关(int accid, int type, int cfg) throws SQLException {
        //type:0设置二密开关 
        int count = 0;
        Connection con = DatabaseConnection.getConnection();
        PreparedStatement ps = null;
        ps = con.prepareStatement("SELECT COUNT(1) FROM playercfg WHERE accid = ? and type = ?");
        ps.setString(1, String.valueOf(accid));
        ps.setString(2, String.valueOf(type));
        ResultSet rs = ps.executeQuery();
        if (rs.next()) {
        ...
        if (count == 1) {
            ps = con.prepareStatement("UPDATE playercfg SET cfg = ? WHERE accid = ? and type = ?");
            ps.setString(1, String.valueOf(cfg));
            ps.setString(2, String.valueOf(accid));
            ps.setString(3, String.valueOf(type));
            ps.executeUpdate();
        } else {
            ps = con.prepareStatement("INSERT INTO playercfg (accid, type, cfg) values (?, ?, ?)");
            ps.setString(1, String.valueOf(accid));
            ps.setString(2, String.valueOf(type));
            ps.setString(3, String.valueOf(cfg));
            ps.executeUpdate();
        }
        rs.close();
        ps.close();
    ...
    public int 查询开关(int accid, int type) throws SQLException {
        int cfg = 1; //0关 1开
        Connection con = DatabaseConnection.getConnection();
        PreparedStatement ps = null;
        ps = con.prepareStatement("SELECT cfg FROM playercfg WHERE accid = ? and type = ?");
    ...
    public boolean checkSerPicOF(){
        return ServerConstants.ENABLE_PIC;
    }
}
