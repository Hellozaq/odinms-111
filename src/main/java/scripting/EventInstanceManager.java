...
public class EventInstanceManager {
    ...
    public EventInstanceManager(EventManager em, String name, int channel) {
        ...
        mapFactory.setInstanceManager(this);
    }
        ...
        try {
            ...
            for (MapleCharacter chr : getPlayers()) {
                if (name.startsWith("PVP")) {
                    chr.getClient().sendPacket(EtcPacket.getPVPClock(Integer.parseInt(getProperty("type")), timesend));
                } else {
                    chr.getClient().sendPacket(EtcPacket.getClock(timesend));
                }
            }
            timeOut(time, this);
    ...
    public void registerSquad(MapleSquad squad, MapleMap map, int questID) {
        ...
        squad.getBeginMap().broadcastMessage(EtcPacket.stopClock());
    }
