...
public class CashShopOperation {
    ...
    public static void LeaveCS(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        try {
            ...
            try {
                c.sendPacket(EtcPacket.getChannelChange(InetAddress.getByName(socket[0]), Integer.parseInt(ChannelServer.getInstance(c.getChannel()).getIP().split(":")[1])));
    ...
    public static final void BuyCashItem(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (action == 0) {
            ...
            if (info == null || info.getLeft().intValue() <= 0 || info.getLeft().intValue() == c.getPlayer().getId() || info.getMid().intValue() == c.getAccID()) {
                ...
                chr.sendNote(partnerName, "收到'" + chr.getName() + "'赠送的" + itemCount + "个[" + getItem(item.getId()) + "]", (byte) 0);
            }
            ...
            MapleQuestStatus marr = c.getPlayer().getQuestNoAdd(MapleQuest.getInstance(GameConstants.PENDANT_SLOT));
            if (marr != null && marr.getCustomData() != null && Long.parseLong(marr.getCustomData()) >= System.currentTimeMillis()) {
                //c.getPlayer().getQuestNAdd(MapleQuest.getInstance(GameConstants.PENDANT_SLOT)).setCustomData("" + (System.currentTimeMillis() + ((sn == 40200048 ? 30 : 7) * 24 * 60 * 60 * 1000)));
                c.getPlayer().getQuestNAdd(MapleQuest.getInstance(GameConstants.PENDANT_SLOT)).setCustomData(String.valueOf(Long.parseLong(marr.getCustomData()) + ((long) 30 * 24 * 60 * 60000)));
                c.getPlayer().modifyCSPoints(toCharge, -3200, false);
                chr.dropMessage(1, "吊墜插槽時間新增了30天。");
            } else {
                ...
                //c.getPlayer().getQuestNAdd(MapleQuest.getInstance(GameConstants.PENDANT_SLOT)).setCustomData("" + (System.currentTimeMillis() + ((sn == 40200048 ? 30 : 7) * 24 * 60 * 60 * 1000)));
                c.getPlayer().modifyCSPoints(toCharge, -3200, false);
                c.getPlayer().pendants = true;
                chr.dropMessage(1, "吊墜插槽數已擴展成功。");
            }
    ...
    private static String getItem(int itemId) {
