...
public class DamageParse {
    ...
    public static void applyAttack(final AttackInfo attack, final Skill theSkill, final MapleCharacter player, int attackCount, final double maxDamagePerMonster, final MapleStatEffect effect, final AttackType attack_type) {
                ...
                if (mapobject != null) {
                    ...
                    try {
                        if (mapitem.getMeso() > 0) {
                            ...
                            map.broadcastMessage(EtcPacket.explodeDrop(mapitem.getObjectId()));
        ...
        for (final AttackPair oned : attack.allDamage) {
            ...
            if (monster != null && monster.getLinkCID() <= 0) {
                    ...
                    if (fixeddmg != -1) {
                        ...
                        if (monsterstats.getOnlyNoramlAttack()) {
                            ...
                            if (Tempest) { // Monster buffed with Tempest
                                ...
                                //       if (eachd > maxDamagePerHit) {//수정
                                //    player.getCheatTracker().registerOffense(CheatingOffense.HIGH_DAMAGE, "[데미지 : " + eachd + ", 예상 데미지 : " + maxDamagePerHit + ",  때린 몹ID : " + monster.getId() + "] [해당 플레이어 직업 : " + player.getJob() + ", 레벨 : " + player.getLevel() + ", 사용스킬 : " + attack.skill + "]");
                                //           if (attack.real) {//수정
                                //       player.getCheatTracker().checkSameDamage(eachd, maxDamagePerHit);
                                //                 }//수정
                                //      if (eachd > maxDamagePerHit * 99999) {
                              ...
                              /*        if (eachd >= 99999999) { //수정했음
                                               // 맥뎀
                                          //  player.getClient().getSession().close(true);//수정했음
                                            return;
                                        }*/
                                //       }
                                //       }//수정
                ...
                if (totDamageToOneMonster > 0 || attack.skill == 1221011 || attack.skill == 21120006) {
                    ...
                    if (attack.skill == 1221009) {  //블래스트 즉사
                        if (Randomizer.isSuccess(SkillFactory.getSkill(1221009).getEffect(player.getSkillLevel(1221009)).getZ())) {
                    ...
                    if (attack.skill == 1321012) {  //다크 임페일 즉사
                        if (Randomizer.isSuccess(SkillFactory.getSkill(1321012).getEffect(player.getSkillLevel(1321012)).getZ())) {
                            if (!monster.getStats().isBoss()) {
                                if (totDamageToOneMonster > 0) {
                                    totDamageToOneMonster = 99999999;
                                }
                    ...
                    if (attack.skill != 1221011) {
                        if (player.isDma())
                            player.dropMessage(-1, "실적용 데미지 : " + totDamageToOneMonster);
                                ...
                                if (skill.getEffect(player.getSkillLevel(c)).makeChanceResult()) {
                                    ...
                                    if (c == 1211006) {
                                        if (player.isBuffFrom(MapleBuffStat.WK_CHARGE, skill)) {
                                            MonsterStatusEffect monsterStatusEffect = new MonsterStatusEffect(MonsterStatus.FREEZE, 1, c, null, false);
        ...
        if (attack.skill == 35111001 || attack.skill == 35111009) {
            if (player.getSkillLevel(attack.skill) < 8) {
                attack.skill = 35111001;
                EnumMap<MapleBuffStat, Integer> stat = new EnumMap<MapleBuffStat, Integer>(MapleBuffStat.class);
                MapleStatEffect ceffect = SkillFactory.getSkill(35111001).getEffect(player.getTotalSkillLevel(SkillFactory.getSkill(35111001)));
                player.getClient().sendPacket(MaplePacketCreator.BuffPacket.giveBuff(35111001, 2100000000, stat, ceffect));
            } else if (player.getSkillLevel(attack.skill) >= 8 && player.getSkillLevel(attack.skill) < 15) {
                attack.skill = 35111009;
                EnumMap<MapleBuffStat, Integer> stat = new EnumMap<MapleBuffStat, Integer>(MapleBuffStat.class);
                MapleStatEffect ceffect = SkillFactory.getSkill(35111009).getEffect(player.getTotalSkillLevel(SkillFactory.getSkill(35111009)));
                player.getClient().sendPacket(MaplePacketCreator.BuffPacket.giveBuff(35111009, 2100000000, stat, ceffect));
            } else {
                attack.skill = 35111010;
                EnumMap<MapleBuffStat, Integer> stat = new EnumMap<MapleBuffStat, Integer>(MapleBuffStat.class);
                MapleStatEffect ceffect = SkillFactory.getSkill(35111010).getEffect(player.getTotalSkillLevel(SkillFactory.getSkill(35111010)));
                player.getClient().sendPacket(MaplePacketCreator.BuffPacket.giveBuff(35111010, 2100000000, stat, ceffect));
            }
        }
...
//			    System.out.println("Client damage : " + eachd + " Server : " + MaxDamagePerHit);
                            ...
                            if (Tempest) { // Buffed with Tempest
                                ...
                                //     if (eachd > MaxDamagePerHit) {//수정
                                //     player.getCheatTracker().registerOffense(CheatingOffense.HIGH_DAMAGE_MAGIC, "[데미지 : " + eachd + ", 예상데미지 : " + MaxDamagePerHit + ", 몹ID : " + monster.getId() + "] [직업 : " + player.getJob() + ", 레벨 : " + player.getLevel() + ", 스킬 : " + attack.skill + "]");
                                //  if (attack.real) {//수정
                                //       player.getCheatTracker().checkSameDamage(eachd, MaxDamagePerHit);
                                //      }//수정
                                //    if (eachd > maxDamagePerHit * 99999) {
                ...
                if (player.getJob() == 212 || player.getJob() == 222 || player.getJob() == 232) {
                    if (attack.skill > 0) {
                        if (player.getSkillLevel(2120010) > 0 || player.getSkillLevel(2220010) > 0 || player.getSkillLevel(2320011) > 0) {
                            if (player.samemobid == 0) player.samemobid = monster.getId();
                            else {
                                if (player.samemobid2 == 0) {
                                    player.samemobid2 = monster.getId();
                                } else {
                                    if (player.samemobid == player.samemobid2) {
                                        if (player.samecount < 1) player.samecount++;
                                        else player.samecount = 1;
                                    } else if (player.samemobid != player.samemobid2) {
                                        player.samemobid = monster.getId();
                                        player.samemobid2 = 0;
                                        player.samecount = 0;
                                    }
                                }
                                if (player.samemobid == player.samemobid2 && player.samecount >= 1) {
                                    if (player.acainaim < 5) player.acainaim += 1;
                                    final Skill skill = SkillFactory.getSkill(2120010);
                                    EnumMap<MapleBuffStat, Integer> stat = new EnumMap<MapleBuffStat, Integer>(MapleBuffStat.class);
                                    MapleStatEffect ceffect = SkillFactory.getSkill(2120010).getEffect(player.getTotalSkillLevel(SkillFactory.getSkill(2120010)));
                                    stat.put(MapleBuffStat.ARCANE_AIM, player.acainaim);
                                    player.getClient().sendPacket(MaplePacketCreator.BuffPacket.giveBuff(2120010, 5000, stat, ceffect));
                                    if (player.acainaimtimer != null) {
                                        player.acainaimtimer.cancel(false);
                                    }
                                    player.acainaimtimer = Timer.WorldTimer.getInstance().schedule(new Runnable() {
                                        public void run() {
                                            player.acainaim = 0;
                                            player.samemobid = 0;
                                            player.samemobid2 = 0;
                                            player.samecount = 0;
                                            player.getMap().broadcastMessage(player, EtcPacket.skillCancel(player, 2120010), false);
                                        }
                                    }, 5000L);
                                }
                            }
