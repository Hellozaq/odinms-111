...
public class SummonHandler {
    ...
    public static void SummonAttack(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        // return;
        //}
        slea.skip((summon.getSkill() == 35111002 || summon.getSkill() == 35111009) ? 24 : 12); //some pos stuff
    ...
    public static final void SubSummon(final LittleEndianAccessor slea, final MapleCharacter chr) {
        ...
        switch (sum.getSkill()) {
            ...
            case 1321007: //beholder
                ...
                if (bHealing.getId() == 1320009) {
                    int type = Randomizer.rand(0, 4);
                    MapleStatEffect bHealingbuf = MapleItemInformationProvider.getInstance().getItemEffectlevel((2022125 + type), chr.getSkillLevel(1320009));
                    bHealingbuf.applyTo(chr);
        ...
        if (GameConstants.isAngel(sum.getSkill())) {
            if (sum.getSkill() % 10000 == 1087) {
                SkillFactory.getSkill(sum.getSkill() - 999).getEffect(1).applyTo(chr);
