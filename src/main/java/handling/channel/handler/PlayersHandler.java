...
public class PlayersHandler {
    ...
    public static void GiveFame(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (target == null || target == chr) { // faming self
            c.sendPacket(MaplePacketCreator.giveFameErrorResponse(1));
            return;
        } else if (chr.getLevel() < 15) {
            c.sendPacket(MaplePacketCreator.giveFameErrorResponse(2));
            return;
        }
        switch (chr.canGiveFame(target)) {
            case OK:
                ...
                c.sendPacket(MaplePacketCreator.OnFameResult(0, target.getName(), famechange == 1, target.getFame()));
                target.getClient().sendPacket(MaplePacketCreator.OnFameResult(5, chr.getName(), famechange == 1, 0));
                break;
            case NOT_TODAY:
            ...
            if (reactor.getReactorType() == 100) {
                ...
                if (c.getPlayer().haveItem(itemid, reactor.getReactItem().getRight())) {
                    if (reactor.getArea().contains(c.getPlayer().getTruePosition())) {
                        ...
                        c.getPlayer().dropMessage(5, "距離太遠了。");
                    }
                } else {
                    c.getPlayer().dropMessage(5, "沒有攜帶所需的物品。");
                }
            } else {
                //just hit it
    ...
    public static void hitCoconut(LittleEndianAccessor slea, MapleClient c) {
        ...
        if (nut.getHits() > 2 && Math.random() < 0.4 && !nut.isStopped()) {
            ...
            if (Math.random() < 0.01 && map.getStopped() > 0) {
                nut.setStopped(true);
                map.stopCoconut();
                c.getPlayer().getMap().broadcastMessage(EtcPacket.hitCoconut(false, id, 1));
                return;
            }
            ...
            if (Math.random() < 0.05 && map.getBombings() > 0) {
                ...
                c.getPlayer().getMap().broadcastMessage(EtcPacket.hitCoconut(false, id, 2));
                map.bombCoconut();
            } else if (map.getFalling() > 0) {
                //System.out.println("Coconut5-2");
                c.getPlayer().getMap().broadcastMessage(EtcPacket.hitCoconut(false, id, 3));
                map.fallCoconut();
                ...
                c.getPlayer().getMap().broadcastMessage(EtcPacket.coconutScore(map.getCoconutScore()));
            }
        } else {
        ...
        if (c.getPlayer().getFollowId() > 0 && c.getPlayer().getFollowId() == slea.readInt()) {
            ...
            if (tt != null && tt.getPosition().distanceSq(c.getPlayer().getPosition()) < 10000 && tt.getFollowId() == 0 && tt.getId() != c.getPlayer().getId()) { //estimate, should less
                ...
                if (accepted) {
                    ...
                    c.getPlayer().getMap().broadcastMessage(EtcPacket.followEffect(tt.getId(), c.getPlayer().getId(), null));
                } else {
                    c.getPlayer().setFollowId(0);
                    tt.setFollowId(0);
                    tt.getClient().sendPacket(EtcPacket.getFollowMsg(5));
                }
            } else {
                if (tt != null) {
                    tt.setFollowId(0);
    ...
    public static void Report(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (other == null || type == null || other.isIntern()) {
            c.sendPacket(MaplePacketCreator.report(4));
            return;
        }
        ...
        if (theTime + 7200000 > currentTime && !c.getPlayer().isIntern()) {
            ...
            c.sendPacket(MaplePacketCreator.report(2));
        }
    }
    ...
    public static final void MonsterBookInfoRequest(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (player != null && !player.isClone()) {
            if (!player.isGM() || c.getPlayer().isGM()) {
                c.sendPacket(MaplePacketCreator.getMonsterBookInfo(player));
            }
        }
    }
    ...
    public static final void MonsterBookDropsRequest(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (mobid <= 0 || !chr.getMonsterBook().hasCard(cardid)) {
            c.sendPacket(MaplePacketCreator.getCardDrops(cardid, null));
            return;
        }
        final MapleMonsterInformationProvider ii = MapleMonsterInformationProvider.getInstance();
        final List<Integer> newDrops = new ArrayList<Integer>();
        ...
        c.sendPacket(MaplePacketCreator.getCardDrops(cardid, newDrops));
    }
    ...
    public static final void ChangeSet(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        c.sendPacket(MaplePacketCreator.changeCardSet(set));
    }
    ...
    public static final void EnterPVP(final LittleEndianAccessor slea, final MapleClient c) {
        if (c.getPlayer() == null || c.getPlayer().getMap() == null || c.getPlayer().getMapId() != 960000000) {
            c.sendPacket(EtcPacket.pvpBlocked(1));
            c.sendPacket(MaplePacketCreator.enableActions());
            return;
        }
        if (c.getPlayer().getParty() != null) {
            c.sendPacket(EtcPacket.pvpBlocked(9));
            c.sendPacket(MaplePacketCreator.enableActions());
            return;
        }
        c.getPlayer().updateTick(slea.readInt());
    ...
    public static final void RespawnPVP(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        c.sendPacket(EtcPacket.getPVPScore(Integer.parseInt(c.getPlayer().getEventInstance().getProperty(String.valueOf(c.getPlayer().getId()))), false));
        ...
        List<Pair<Integer, String>> players1 = new LinkedList<Pair<Integer, String>>();
        for (int xx = 0; xx < players.size(); xx++) {
            players1.add(new Pair<Integer, String>(players.get(xx).getId(), players.get(xx).getName()));
        }
        c.sendPacket(EtcPacket.getPVPType(type, players1, c.getPlayer().getTeam(), true, lvl));
        c.sendPacket(EtcPacket.enablePVP(true));
    }
    ...
    public static final void LeavePVP(final LittleEndianAccessor slea, final MapleClient c) {
        if (c.getPlayer() == null || c.getPlayer().getMap() == null || !c.getPlayer().inPVP()) {
            c.sendPacket(EtcPacket.pvpBlocked(6));
            c.sendPacket(MaplePacketCreator.enableActions());
            return;
        }
        ...
        c.getPlayer().clearAllCooldowns();
        c.getPlayer().updateTick(slea.readInt());
        c.sendPacket(MaplePacketCreator.clearMidMsg());
    ...
    public static final void AttackPVP(final LittleEndianAccessor slea, final MapleClient c) {
            ...
            switch (chr.getJob()) {
                ...
                case 1112:
                    if (PlayerHandler.isFinisher(skillid) > 0) { // finisher
                        ...
                        chr.setBuffedValue((MapleBuffStat.COMBO), PlayerHandler.isFinisher(skillid));
        ...
        if (!area) {
            ...
            for (MapleCharacter attacked : chr.getMap().getCharactersIntersect(box)) {
                if (attacked.getId() != chr.getId() && attacked.isAlive() && !attacked.isHidden() && (type == 0 || attacked.getTeam() != chr.getTeam())) {
                    ...
                    if (ice == attacked.getId()) {
                        chr.getClient().sendPacket(EtcPacket.getPVPIceHPBar(attacked.getStat().getHp(), attacked.getStat().getCurrentMaxHp()));
                    } else {
                        chr.getClient().sendPacket(EtcPacket.getPVPHPBar(attacked.getId(), attacked.getStat().getHp(), attacked.getStat().getCurrentMaxHp()));
                    }
            ...
            if (Integer.parseInt(chr.getEventInstance().getProperty("redflag")) == chr.getId() && chr.getMap().getArea(1).contains(chr.getTruePosition())) {
                ...
                chr.getEventInstance().broadcastPacket(EtcPacket.getCapturePosition(chr.getMap()));
                chr.getEventInstance().broadcastPacket(EtcPacket.resetCapture());
        ...
        if (killed || addedScore > 0) {
            ...
            chr.getClient().sendPacket(EtcPacket.getPVPScore(ourScore + addedScore, killed));
        }
        if (didAttack) {
        ...
        chr.getMap().broadcastMessage(EtcPacket.pvpAttack(chr.getId(), chr.getLevel(), trueSkill, trueSkillLevel, speed, fakeMastery, visProjectile, attackCount, chargeTime, animation, facingLeft ? 1 : 0, chr.getStat().defRange, skillid, skillLevel, move, push, pull, ourAttacks));
    ...
    public static void TeachSkill(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        if ((quest != null) && (chr.getAccountID() == toChrAccId)) {
            ...
            if ((chr.teachSkill(toSkillId, toChrId) > 0) && (toSkillId >= 80000000)) {
                ...
                c.sendPacket(MaplePacketCreator.teachMessage(skillId, toChrId, toChrName));
            } else {
