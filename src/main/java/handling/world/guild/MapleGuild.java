...
public class MapleGuild implements java.io.Serializable {
    ...
    public boolean init = false, changed = false, changed_skills = false;
    ...
    public MapleGuild(final int guildid, Map<Integer, Map<Integer, MapleBBSReply>> replies) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                guildSkills.put(sid, new MapleGuildSkill(sid, rs.getInt("level"), rs.getLong("timestamp"), rs.getString("purchaser"), ""));
            }
        ...
        for (final MapleGuildCharacter member : members) {
            if (member.getId() == mgc.getId()) {
                ...
                if (old_level != mgc.getLevel()) {
                    ...
                    this.broadcast(MaplePacketCreator.serverNotice(5, "<家族> " + mgc.getName() + "已达到" + mgc.getLevel() + "级。"));
                }
                if (old_job != mgc.getJobId()) {
                    //this.broadcast(CWvsContext.sendJobup(false, mgc.getJobId(), mgc.getName()), mgc.getId());
                    this.broadcast(MaplePacketCreator.serverNotice(5, "<家族> " + mgc.getName() + "转职为" + GameConstants.getJobName(mgc.getJobId()) + "。"));
                }
