    ...
    public final void memberLevelJobUpdate(final MapleCharacter mgc) {
        ...
        if (member != null) {
            ...
            if (old_level != mgc.getLevel()) {
                ...
                this.broadcast(MaplePacketCreator.serverNotice(5, "< 家族 > " + mgc.getName() + "提升至" + mgc.getLevel() + "等級，讓我們為她加油吧！"), mgc.getId(), mgc.getId() == leaderid ? null : member.getPedigree());
            }
            if (old_job != mgc.getJob()) {
                ...
                this.broadcast(MaplePacketCreator.serverNotice(5, "< 家族 > " + mgc.getName() + "升職為" + GameConstants.getJobName((int) mgc.getJob()) + "，讓我們祝賀她。"), mgc.getId(), mgc.getId() == leaderid ? null : member.getPedigree());
            }
        }
    }
