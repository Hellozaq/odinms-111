...
function enter(pi) {
    ...
    for (var i = 1; i < 9; i++) {
        ...
        if (eim.getProperty(i + "st") == x) {
            pi.getClient().sendPacket(Packages.tools.packet.EtcPacket.instantMapWarp(pi.getPlayer().getMap().getPortal("np0" + (pi.getPortal().getName().substring(3, 4) - 1)).getId()));
    ...
    pi.getClient().sendPacket(Packages.tools.packet.EtcPacket.instantMapWarp(pi.getPlayer().getMap().getPortal(x.substring(2, 5) > 54 ? "np03" : "np16").getId()));
