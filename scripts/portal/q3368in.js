...
function enter(pi) {
    ...
    if (pi.getPlayer().getQuestNAdd(Packages.server.quest.MapleQuest.getInstance(3368)).getStatus() != 1) {
        pi.getClient().sendPacket(Packages.tools.packet.MaplePacketCreator.serverNotice(6, "猶利塔的實驗室3禁止外來者訪問"));
        return false;
    }
    if (pi.getMap(926130103).getCharacters().size() > 0) {
        pi.getClient().sendPacket(Packages.tools.packet.MaplePacketCreator.serverNotice(6, "猶利塔的實驗室3目前擁擠，請稍後再試"));
        return false;
    }
