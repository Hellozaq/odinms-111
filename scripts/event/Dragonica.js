...
function monsterValue(eim, mobId) { //殺怪後觸發
    if (mobId == 8300006) {
        eim.getMapInstance(240080600).broadcastMessage(Packages.tools.packet.MaplePacketCreator.serverNotice(6, "夢幻龍族已被消滅"));
        eim.getMapInstance(240080600).broadcastMessage(Packages.tools.packet.EtcPacket.environmentChange("quest/party/clear", 3));
        eim.getMapInstance(240080600).broadcastMessage(Packages.tools.packet.EtcPacket.environmentChange("Party1/Clear", 4));
    }
    if (mobId == 8300007) {
        eim.startEventTimer(3 * 60000);
        eim.getMapInstance(240080800).spawnNpc(2085003, new java.awt.Point(400, -10));
        eim.getMapInstance(240080800).broadcastMessage(Packages.tools.packet.MaplePacketCreator.serverNotice(6, "禦龍魔已被消滅"));
        eim.getMapInstance(240080800).broadcastMessage(Packages.tools.packet.EtcPacket.environmentChange("quest/party/clear", 3));
        eim.getMapInstance(240080800).broadcastMessage(Packages.tools.packet.EtcPacket.environmentChange("Party1/Clear", 4));
    }
    return 1;
}
