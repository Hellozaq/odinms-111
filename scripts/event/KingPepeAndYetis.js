...
function playerEntry(eim, player) { //傳送進事件地圖
    ...
    map.broadcastMessage(Packages.tools.packet.EtcPacket.environmentChange(mapEff[num], 3));
    map.broadcastMessage(Packages.tools.packet.EtcPacket.environmentChange("pepeKing/chat/nugu", 3));
}
...
function monsterValue(eim, mobId) { //殺怪後觸發
    if (mobId == 3300005 || mobId == 3300006 || mobId == 3300007) {
        eim.getMapInstance(106021500).broadcastMessage(Packages.tools.packet.MaplePacketCreator.serverNotice(6, "雪吉拉和企鵝國王被打敗了"));
        eim.getMapInstance(106021500).broadcastMessage(Packages.tools.packet.EtcPacket.environmentChange("quest/party/clear", 3));
        eim.getMapInstance(106021500).broadcastMessage(Packages.tools.packet.EtcPacket.environmentChange("Party1/Clear", 4));
