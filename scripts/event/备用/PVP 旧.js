    ...
    if (eim.getProperty("started").equals("0")) {
        ...
        if (eim.getPlayerCount() >= (player.isGM() ? 2 : i)) {
            ...
            if (type == 2) {
                ...
                ice.changeTeam(1);
                //eim.applySkill(ice, ice.getStat().getSkillByJob(1105, ice.getJob()));//報錯，暫時注釋
        ...
        if (type != 0) {
            player.getClient().sendPacket(EtcPacket.getPVPPoints(parseInt(eim.getProperty("red")), parseInt(eim.getProperty("blue"))));
        }
        player.getClient().sendPacket(EtcPacket.getPVPScore(0, false));
    }
}
...
function addPVPScore(eim, player, score) {
    ...
    if (parseInt(eim.getProperty("ice")) == player.getId()) {
        ...
        player.getClient().sendPacket(EtcPacket.getPVPIceGage(xx));
...
function playerDead(eim, player) {
    player.getClient().sendPacket(EtcPacket.getPVPKilled(player.getName() + " has been defeated."));
    if (parseInt(eim.getProperty("ice")) == player.getId()) {
        eim.setProperty("ice", "0");
...
function championCheck(eim) {
    ...
    if (champion != null) {
        ...
        champion.getClient().sendPacket(EtcPacket.showOwnChampionEffect());
...
function updateScoreboard(eim, score) {
    ...
    if (score == true || score == null) {
        ...
        for (var i = 0; i < players.size(); i++) {
            ...
            players.get(i).getClient().sendPacket(EtcPacket.getPVPResult(c, exp, winningTeam + 1, players.get(i).getTeam() + 1));

            Qa = parseInt(players.get(i).getQuestNAdd(MapleQuest.getInstance(11054)).getCustomData());
            if (players.get(i).getQuestStatus(11051) == 1 && players.get(i).getQuestNAdd(MapleQuest.getInstance(11054)).getCustomData() < 5) {
                players.get(i).getQuestNAdd(MapleQuest.getInstance(11054)).setCustomData(Qa + 1);
                players.get(i).updateQuest(players.get(i).getQuestNAdd(MapleQuest.getInstance(11054)), true);
            }
            if (players.get(i).getQuestStatus(11052) == 1 && x > 1000 && ty == 0) {
                players.get(i).getQuestNAdd(MapleQuest.getInstance(11055)).setCustomData(1000);
                players.get(i).updateQuest(players.get(i).getQuestNAdd(MapleQuest.getInstance(11055)), true);
            }
            Qs = parseInt(players.get(i).getQuestNAdd(MapleQuest.getInstance(11056)).getCustomData());
            if (players.get(i).getQuestStatus(11053) == 1 && players.get(i).getQuestNAdd(MapleQuest.getInstance(11056)).getCustomData() < 3 && ty == 1 && players.get(i).getTeam() == winningTeam) {
                players.get(i).getQuestNAdd(MapleQuest.getInstance(11056)).setCustomData(Qs + 1);
                players.get(i).updateQuest(players.get(i).getQuestNAdd(MapleQuest.getInstance(11056)), true);
            }
            if (players.get(i).getQuestStatus(11274) == 1 && ty == 2 && players.get(i).getTeam() == winningTeam && parseInt(eim.getProperty("ice")) == players.get(i).getId()) {
                players.get(i).getQuestNAdd(MapleQuest.getInstance(11059)).setCustomData(1);
                players.get(i).updateQuest(players.get(i).getQuestNAdd(MapleQuest.getInstance(11059)), true);
            }
        }
    }
}
...
function playerExit(eim, player) {
    player.getClient().sendPacket(MaplePacketCreator.clearMidMsg());
...
function end(eim) {
    ...
    var players = eim.getPlayers();
    for (var i = 0; i < players.size(); i++) {
        players.get(i).getClient().sendPacket(MaplePacketCreator.clearMidMsg());
