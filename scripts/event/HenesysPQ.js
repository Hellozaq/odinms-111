...
function checkHogHealth(eim) { //監控地图血量
    ...
    if (watchHog != null) {
        ...
        if (oldHp - hp > 300) { // or 400, if using mobHP / eventTime
            eim.getMapInstance(910010000).broadcastMessage(Packages.tools.packet.MaplePacketCreator.serverNotice(6, "月妙感到不舒服，請保護它，這樣它就可以做美味的年糕了"));
        }
...
function monsterValue(eim, mobId) { //殺怪後觸發
    if (mobId == 9300061) {
        eim.getMapInstance(910010000).broadcastMessage(Packages.tools.packet.MaplePacketCreator.serverNotice(6, "好可惜，沒有保護好月妙"));
