...
function monsterValue(eim, mobId) { //殺怪後觸發
    if ((mobId == 9300450 || mobId == 9300451) && eim.getMapInstance(921160200).getAllMonstersThreadsafe().size() < 1 && eim.getProperty("stage2") == null) {
        eim.getMapInstance(921160200).broadcastMessage(Packages.tools.packet.MaplePacketCreator.serverNotice(6, "通往下一區域的入口已經開啟"));
    ...
    if ((mobId == 9300450 || mobId == 9300452) && eim.getMapInstance(921160400).getAllMonstersThreadsafe().size() < 1 && eim.getProperty("stage4") == null) {
        eim.getMapInstance(921160400).broadcastMessage(Packages.tools.packet.MaplePacketCreator.serverNotice(6, "通往下一區域的入口已經開啟"));
    ...
    if (mobId == 9300461) {
        eim.getMapInstance(921160700).broadcastMessage(Packages.tools.packet.MaplePacketCreator.serverNotice(6, "教官阿尼被消滅了，快去拯救人质"));
...
function scheduledTimeout(eim) { //規定時間結束
    if (eim.getProperty("kentaSaving") != null && eim.getProperty("kentaSaving") != 0) {
        ...
        eim.getMapInstance(921160600).broadcastMessage(Packages.tools.packet.MaplePacketCreator.serverNotice(6, "通往下一區域的入口已經開啟"));
