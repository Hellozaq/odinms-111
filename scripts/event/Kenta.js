...
function checkHogHealth(eim) { //監控地图血量
    ...
    if (watchHog != null) {
        ...
        if (oldHp - hp > 700) {
            eim.getMapInstance(923040300).broadcastMessage(Packages.tools.packet.MaplePacketCreator.serverNotice(6, "請保護坎特不受到傷害"));
        }
...
function monsterValue(eim, mobId) { //殺怪後觸發
    if ((mobId == 9300444 || mobId == 9300445) && eim.getMapInstance(923040100).getAllMonstersThreadsafe().size() == 0 && eim.getProperty("limit") == null) {
        eim.getMapInstance(923040100).broadcastMessage(Packages.tools.packet.MaplePacketCreator.serverNotice(6, "通往下一區域的入口已經開啟"));
    ...
    if (mobId == 9300460) {
        eim.getMapInstance(923040300).broadcastMessage(Packages.tools.packet.MaplePacketCreator.serverNotice(6, "拯救半人馬失敗"));
    ...
    if (eim.getProperty("stage") == 2) {
        ...
        eim.getMapInstance(923040400).broadcastMessage(Packages.tools.packet.MaplePacketCreator.serverNotice(6, "海怒斯被打敗了"));
