...
function setup(mapid) { //開始事件，時間
    ...
    eim.setInstanceMap((map - 30000) + 2);
...
function registerCarnivalParty(eim, carnivalParty) { //等待區設置
    ...
    eim.getMapInstance(waitingMap).broadcastMessage(Packages.tools.packet.MaplePacketCreator.serverNotice(6, "怪物擂臺賽Ⅱ即將開始"));
...
function playerExit(eim, player) { //角色退出時觸發
    ...
    if (eim.getProperty("started") != 3) {
        player.getClient().sendPacket(Packages.tools.packet.MaplePacketCreator.serverNotice(6, player.getName() + "離開了怪物擂臺賽Ⅱ區域，本次比賽已取消"));
        eim.disposeIfPlayerBelow(100, 980030000);
    }
}
