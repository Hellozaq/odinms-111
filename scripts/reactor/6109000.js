...
function act() {
    switch (rm.getPlayer().getMap().getId()) {
        case 610030200:
            ...
            rm.getPlayer().getMap().broadcastMessage(Packages.tools.packet.MaplePacketCreator.serverNotice(6, "戰士印記已激活！還差" + (5 - nun) + "個印記啟動，才能開啟傳送門"));
            if (nun > 4) {
            ...
            break;
        case 610030300:
            rm.getPlayer().getMap().moveEnvironment("menhir0", 1);
            ...
            rm.getPlayer().getMap().broadcastMessage(Packages.tools.packet.MaplePacketCreator.serverNotice(6, "戰士印記已激活！区域的防禦系統開始運作！還差" + (5 - nun) + "個印記啟動，才能開啟底部的傳送門"));
            if (nun > 4) {
                rm.getPlayer().getMap().broadcastMessage(Packages.tools.packet.MaplePacketCreator.serverNotice(6, "通往下一區域的入口，已經開啟"));
