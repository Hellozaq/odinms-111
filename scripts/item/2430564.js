...
function start() {
    ...
    if (im.getPlayer().getMap().getAllMonstersThreadsafe().size() > 0) {
        im.getClient().sendPacket(Packages.tools.packet.MaplePacketCreator.serverNotice(5, "Unable to use due to monster obstruction."));
        im.dispose();
        return;
    }
    ...
    im.getClient().sendPacket(Packages.tools.packet.MaplePacketCreator.serverNotice(5, "The magic of the Elven Light cleared the obstacles."));
    im.dispose();
}
