...
function start() {
    if (im.getPlayer().getQuestNAdd(Packages.server.quest.MapleQuest.getInstance(31152)).getStatus() != 2) {
        im.getClient().sendPacket(Packages.tools.packet.MaplePacketCreator.serverNotice(5, "You have no idea how to use it."));
        im.dispose();
        return;
    }
    ...
    if (!(im.getPlayer().itemQuantity(4000660) && im.getPlayer().itemQuantity(4000661) && im.getPlayer().itemQuantity(4000662) && im.getPlayer().itemQuantity(4000663))) {
        im.getClient().sendPacket(Packages.tools.packet.MaplePacketCreator.serverNotice(5, "There needs to be 1 of each Stone for a Dream Key."));
        im.dispose();
        return;
    }
