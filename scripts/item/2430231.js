...
function start() {
    ...
    if (time + (1 * 60000) > im.getCurrentTime()) {
        im.getClient().sendPacket(Packages.tools.packet.MaplePacketCreator.serverNotice(5, "You can only use one energy drink per 10 minutes."));
        im.dispose();
        return;
    }
    if (im.getPlayer().getFatigue() < 1) {
        im.getClient().sendPacket(Packages.tools.packet.MaplePacketCreator.serverNotice(5, "The character is full of energy and does not need to be used."));
        im.dispose();
        return;
    }
