...
function start() {
    ...
    if (im.getPlayer().itemQuantity(4000630) < 50) {
        im.getClient().sendPacket(Packages.tools.packet.MaplePacketCreator.serverNotice(5, "There needs to be 50 Purification Totems for a Noble Lion King Medal, 100 for Royal Lion King Medal."));
        im.dispose();
        return;
    }
